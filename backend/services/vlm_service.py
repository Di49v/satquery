...
class VLMService:
    ...
    def run_geochat(cls, image_url: Optional[str], query: str, lat: float, lon: float) -> Dict[str, Any]:
        ...
        Falls back to local UI sandbox synthesis if the endpoint disconnects or is unavailable.
        """
        if not image_url:
            return {
                "model": "GeoChat-7B",
        ...
        try:
            ...
            # Catches all network drops, disconnections, timeouts, and server errors universally
            return {
                "model": "GeoChat-7B (Local Sandbox Fallback)",
                "status": "fallback",
                "output": (
                    "Visual inference unavailable. No evidence-backed answer generated."
                )
            }
